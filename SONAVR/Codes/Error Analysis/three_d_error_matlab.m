...
for z = 600:-50:50
    ...
    Results(counter).TargetLocation = pos_target;
    Results(counter).EstimatedLocation = avg_estim_target_pos;
    Results(counter).MaximumDistance = max_error_dist;
    Results(counter).MinimumDistance = min_error_dist;
    Results(counter).AngleVariations = rad2deg(max_psi);
...
filename = 'three_d_error_vars.xlsx';
cd 'D:\Rijul\Git Arena Delhi\VR_3D_Motion_Perception\SONAVR\Codes\Error Analysis\'; %For IIT Computer only
excel_row = ['B146'];
